#ifndef _CBossEarthAI_H_
#define _CBossEarthAI_H_

#include "IBaseAIState.h"

class CBossEarthAI : public IBaseAIState
{
	// Trilogy for proper singleton
	CBossEarthAI(const CBossEarthAI&){}
	CBossEarthAI* operator=(const CBossEarthAI&){}
	~CBossEarthAI(void){}
	CBossEarthAI(void){}

	static CBossEarthAI* sm_pInstance;

	//int m_sndLarvaHiss;

	//	0 - GetWhacked
	//	1 - WhackedPause
	//	2 - SittingStill
	//	3 - SquirmForward
	//	4 - SquirmPause
	//	5 - Lunge

public:
	static IBaseAIState* GetInstance(void);
	void Enter(CBaseCharacter* pCharacter);
	void Update(CBaseCharacter* pCharacter, float fElapsedTime);
	void Exit(CBaseCharacter* pCharacter);
	static void DeleteInstance(void);
};

#endif