#include "StdAfx.h"
#include "../StdAfx.h"
#include "CCricketAIState.h"
#include "../Game Objects/CPlayer.h"

// Singleton's instance
CCricketAIState* CCricketAIState::sm_pInstance = NULL;

IBaseAIState* CCricketAIState::GetInstance(void)
{
	if(!sm_pInstance)
		sm_pInstance = new CCricketAIState();
	return sm_pInstance;
}

void CCricketAIState::DeleteInstance(void)
{
...
void CCricketAIState::Enter(CBaseCharacter* pCharacter)
{
	...
	pCharacter->SetBehavior(CBaseCharacter::BEHAVIOR_CRICKET);
...
void CCricketAIState::Update(CBaseCharacter* pCharacter, float fElapsedTime)
{
	switch( pCharacter->GetMiniState() )
	{
		...
		case 3:	// 3 - Squirm toward the player!
		{
			...
			else
			{
				...
				if( (CPlayer::GetInstance()->GetPosY() >= pCharacter->GetPosY() - 400 && CPlayer::GetInstance()->GetPosY() <= pCharacter->GetPosY() + 400) && (CPlayer::GetInstance()->GetPosX() >= pCharacter->GetPosX() - 400 && CPlayer::GetInstance()->GetPosX() <= pCharacter->GetPosX() + 400) )
				{
					pCharacter->SetMoveTimer( 0.0f );
		...
		case 4:	//	Pause a little, after moving, like a freakin' caterpillar, bro-dawg.
		{
			...
			else if( pCharacter->GetMoveTimer() < 1.0f )	//	I will take a break for this long.
			{
				pCharacter->SetMoveTimer( pCharacter->GetMoveTimer() + fElapsedTime );	//	Increment timer.
			}
			else
		...
		case 5:		// LUNGE OF DEATH
		{
			...
			else if( pCharacter->GetMoveTimer() < 0.3f )	//	I will lunge for this long
			{
				pCharacter->SetMoveTimer( pCharacter->GetMoveTimer() + fElapsedTime );	//	Increment timer.
			}
			else
...
void CCricketAIState::Exit(CBaseCharacter* pCharacter)
{
