#ifndef _CBossDaggerAI_H_
#define _CBossDaggerAI_H_

#include "IBaseAIState.h"

class CBossDaggerAI : public IBaseAIState
{
	// Trilogy for proper singleton
	CBossDaggerAI(const CBossDaggerAI&){}
	CBossDaggerAI* operator=(const CBossDaggerAI&){}
	~CBossDaggerAI(void){}
	CBossDaggerAI(void){}

	static CBossDaggerAI* sm_pInstance;

	//int m_sndLarvaHiss;

	//	0 - GetWhacked
	//	1 - WhackedPause
	//	2 - SittingStill
	//	3 - SquirmForward
	//	4 - SquirmPause
	//	5 - Lunge

public:
	static IBaseAIState* GetInstance(void);
	void Enter(CBaseCharacter* pCharacter);
	void Update(CBaseCharacter* pCharacter, float fElapsedTime);
	void Exit(CBaseCharacter* pCharacter);
	static void DeleteInstance(void);
};

#endif